...
| **Admin Authorization** | Strictly verified DB role (`admin` or `moderator`); no domain or enabler escalation bypasses | Enforced in `server/middleware/auth.js` (`requireAdmin`) | None (closed) | Security Lead | Test suite verifies escalation denial for unverified emails, arbitrary domains, and `enabler` roles |
