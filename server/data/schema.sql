...
CREATE TABLE IF NOT EXISTS profiles (
    ...
    primary_domain VARCHAR(64) NOT NULL CHECK (primary_domain IN ('climate', 'health', 'education', 'civic', 'hardware', 'ai', 'design')),
...
CREATE TABLE IF NOT EXISTS build_calls (
    ...
    domain VARCHAR(64) NOT NULL CHECK (domain IN ('climate', 'health', 'education', 'civic', 'hardware', 'ai', 'design')),
...
VALUES 
    ...
    ('usr_aarushi', 'canopy.connect.collaborate@gmail.com', 'builder', 'Aarushi Chatterjee', true, '2026-01-01T00:00:00Z', '2026-01-01T00:00:00Z')
...
VALUES
    ('call_groundwater', 'usr_water_ngo', 'Groundwater contamination sensor', 'Rural Water Alliance', 'Build a low-cost optical sensor that flags chemical contamination in real time. Pilot budget and an open dataset are already secured.', 'climate', 'Working optical sensing probe schematic and real-time telemetry firmware.', '$8,500 deployment grant', 'https://data.canopy.internal/sets/gw-sensor-2026', ARRAY['Hardware', 'Embedded C', 'Chemistry', 'LoRa'], 'open'),
    ('call_budget', 'usr_aarushi', 'A plain-language city budget explorer', 'Civic Open Data Collective', 'Municipal budget PDFs are technically public but unreadable to everyday citizens. We need an interactive reconciliation explorer.', 'civic', 'Interactive line-item difference engine and visual allocation treemap.', '$5,000 micro-grant', 'https://data.canopy.internal/sets/city-budget-csv', ARRAY['Data Viz', 'Full-stack', 'Civic Policy'], 'open'),
    ('call_tutor', 'usr_aarushi', 'Adaptive reading tutor for grade 3–5', 'Rural Literacy Foundation', 'Low-latency phonics recognition capable of running fully offline on low-spec donated classroom Android tablets.', 'education', 'Offline WebAssembly phonics scoring engine with touch-first accessibility.', '$6,000 pilot support', 'https://data.canopy.internal/sets/phonics-audio-sample', ARRAY['Full-stack', 'WebAssembly', 'Audio DSP'], 'open'),
    ('call_deforestation', 'usr_maya', 'Deforestation tracker from satellite feeds', 'Canopy Forest Watch', 'Automated weekly Sentinel-2 change detection pipeline flagging unauthorized road-building canopy incisions within 48 hours.', 'ai', 'PyTorch/ONNX inference worker emitting geoJSON bounding alerts.', '$10,000 compute credits', 'https://data.canopy.internal/sets/sentinel-slices', ARRAY['Computer Vision', 'PyTorch', 'GIS', 'Python'], 'open'),
    ('call_solar', 'usr_elena', 'Fault detection for village solar microgrids', 'SunShare Energy Access', 'Current inverter fault logs are siloed and unmonitored. We need a plug-and-play current sensing clip that alerts local technicians before battery banks drain.', 'hardware', 'ESP32 current-loop clamp sensor and alert daemon.', '$7,200 hardware stipend', 'https://data.canopy.internal/sets/inverter-telemetry', ARRAY['Hardware', 'Firmware', 'Power Electronics'], 'open'),
    ('call_triage', 'usr_aarushi', 'Offline sync architecture for clinic triage', 'Frontier Health Labs', 'Rural clinics operate with intermittent 2G coverage. Patient intake records collide and overwrite when reconnecting to central servers.', 'health', 'Conflict-free replicated data types (CRDT) sync client in TypeScript.', '$9,000 deployment pilot', 'https://data.canopy.internal/sets/clinic-records-anon', ARRAY['CRDTs', 'TypeScript', 'Offline First', 'Security'], 'open')
ON CONFLICT (id) DO NOTHING;
